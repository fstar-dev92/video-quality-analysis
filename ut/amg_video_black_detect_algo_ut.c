...
int main(int argc, char *argv[])
{
    ...
    if (argc < 8)
    {
        printf("Usage: %s <infile> <wo> <ho> <skip_roi> <skip_line> <video_file_name>\n", argv[0]);
        exit(1);
    }
    ...
    void *algo_handle = amg_video_black_algo_init(&algo_init_params);
    ...
    algo_params.wo = wo;
    AmgErrorStatus output = amg_video_black_algo_detect(algo_handle,&algo_params);
    printf("file_name: %s, is_black %d\n", video_file, output);
    fprintf(f1, " %s, %d\n", video_file, output);
    
    amg_video_black_algo_deinit(algo_handle);
    fclose(Fin);
    fclose(f1);
